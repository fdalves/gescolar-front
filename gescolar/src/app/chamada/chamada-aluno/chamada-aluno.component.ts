...
export class ChamadaAlunoComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.status = [{ label: "Presença", value: "true" }, { label: "Faltas", value: "false" }]
    }
    ...
    pesquisar(form: FormControl) {
        ...
        chamada.codigoAluno = this.alunoSelecionado;
        chamada.codigoDisciplinaTurma = this.disciplinaSelecionada;
        chamada.dataIni = this.value;
        chamada.dataFim = this.value2;
        chamada.status = this.statusSelecionado;
        this.chamadaService.pesquisaChamadaAluno(chamada)
            .then(chamadas => {
